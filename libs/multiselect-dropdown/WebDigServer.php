	...
	try {
		...
		if( $Identifier_of_new_item_already_exists ) { // it is a request to save a new item, but the identifier already exists in the db
			...
			if( $item_idx >= 0 ) { // it is a request to alter an existing item
				...
				// ==== update the "Square" field of child elements if necessary
				$Square_field_changed = false;
				if ( isset($newData["Square"]) ) { // check if value of the square field has changed
					if( strcmp($ExcData[$item_idx]["IdentifierUUID"], $newData["IdentifierUUID"]) == 0 ) {
						if ( isset($ExcData[$item_idx]["Square"]) == false ) {
							$Square_field_changed = true;
						} else if( strcmp($ExcData[$item_idx]["Square"], $newData["Square"]) != 0 ) {
							$Square_field_changed = true;
						}
					}
					// change the Square filed of all child elements of the altered item
					if( $Square_field_changed ) {
						for ($i=0; $i<count($ExcData); $i++) {
