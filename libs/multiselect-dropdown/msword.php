...
/**
  ...
  * @arg $TrenchName (String) The trench name to be processed.
  ...
  */
function GenerateWordDocument($TrenchName, $UUIDS_to_export, $FileName) {
	// load trench data
	$jsonString = file_get_contents("TrenchData/" . $TrenchName . "/" .   $TrenchName . ".json");
	...
	// define table styles
	$tableStyle = array( 'cellMarginRight' => 200 );
		...
		if( strlen($UUIDS_to_export) <= 3  ||  str_contains($UUIDS_to_export, ",".$TrenchData[$i]["IdentifierUUID"].",") ) { // ignore non selected items
			...
			if ( isset($TrenchData[$i]["RelationIncludesUUID"]) && count($TrenchData[$i]["RelationIncludesUUID"])>0) {
				...
				for ($j = 0; $j<count($IncludedUUIDs); $j++) {	
					$image_filename = "TrenchData/" . $TrenchName . "/images/thumbnails/" . $IncludedUUIDs[$j] . ".jpg";
			...
			$table = $section->addTable('GlobalTableStyle');
			$table->addRow();		$cell=$table->addCell(); $cell->addText("Identifier","GlobalFont_bold"); 	$cell=$table->addCell(); $cell->addText($the_Identifier,"GlobalFont_normal");
			$table->addRow();		$cell=$table->addCell(); $cell->addText("Title","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($TrenchData[$i]["Title"],"GlobalFont_normal");
			if ( isset($TrenchData[$i]["RelationBelongsToUUID"]) && count($TrenchData[$i]["RelationBelongsToUUID"])>0 ) {
				if( strpos($TrenchData[$i]["RelationBelongsToUUID"][0], "\\n") < 0 ) {
					...
					$table->addRow();	$cell=$table->addCell(); $cell->addText("Locus","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($the_Identifier.", ".$ItemData["Title"], "GlobalFont_normal");
				}
			}
			
			if ( isset($TrenchData[$i]["Square"]) ) {
				$table->addRow();	$cell=$table->addCell(); $cell->addText("Square","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($TrenchData[$i]["Square"],"GlobalFont_normal");
			}
			if ( isset($TrenchData[$i]["Description"]) ) {
				$s = $TrenchData[$i]["Description"];
				//$s = substr($s, 0, 682);  // 675 - 682
				//$s = str_replace( "&", "", $s );
				$table->addRow();	$cell=$table->addCell(); $cell->addText("Description","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($s,"GlobalFont_normal");
			}
			if ( isset($TrenchData[$i]["Dimensions"]) ) {
				$table->addRow();	$cell=$table->addCell(); $cell->addText("Dimensions","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($TrenchData[$i]["Dimensions"],"GlobalFont_normal");
			}
			if ( isset($TrenchData[$i]["ArtifactDate"]) &&  strlen($TrenchData[$i]["ArtifactDate"]) > 0 ) {
				$table->addRow();	$cell=$table->addCell(); $cell->addText("Date","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($TrenchData[$i]["ArtifactDate"],"GlobalFont_normal");
			}
			if ( isset($TrenchData[$i]["Comparanda"]) ) {
				$table->addRow();	$cell=$table->addCell(); $cell->addText("Comparanda","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($TrenchData[$i]["Comparanda"],"GlobalFont_normal");
			}
			if ( isset($TrenchData[$i]["Notes"]) ) {
				$table->addRow();	$cell=$table->addCell(); $cell->addText("Notes","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($TrenchData[$i]["Notes"],"GlobalFont_normal");
			}
			if ( isset($TrenchData[$i]["Additional Bibliography"]) ) {
				$table->addRow();	$cell=$table->addCell(); $cell->addText("Additional Bibliography","GlobalFont_bold"); 		$cell=$table->addCell(); $cell->addText($TrenchData[$i]["Additional Bibliography"],"GlobalFont_normal");
			}
			// change page
